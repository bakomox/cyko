...
begin
  ...
  begin
    frmMain.lstOut.Items.add('-i ' + '"' +
      ...
      '" ' + frmMain.venString + trim(frmMain.txtVideoValue.Text) + frmMain.EncoderString + ' -m -E fdk_haac --mixdown stereo -B ' + trim(frmMain.txtAudioValue.Text) +
...
begin
  ...
  begin
    EncoderString := ' -e x264';
  end

  else if (cboEncoder.ItemIndex = 1) then
  begin
    EncoderString := ' -e x265';
  end;
  ...
  try
    if (StrToInt(txtAudioValue.Text) < 32) then
    begin
      ShowMessage('Audio bitrate must not be less than 32');
      txtAudioValue.Text := '';
  ...
  begin
       ...
       begin
            EncoderString := EncoderString + ' ' + '--encoder-profile baseline';
       end
       else if cboProfiles.ItemIndex = 1 then
       begin
            EncoderString := EncoderString + ' ' + '--encoder-profile main';
       end
       else if cboProfiles.ItemIndex = 2 then
       begin
            EncoderString := EncoderString + ' ' + '--encoder-profile high';
       end;
  ...
  begin
       ...
       begin
            EncoderString := EncoderString + ' ' + '--encoder-profile mainstillpicture';
       end;
